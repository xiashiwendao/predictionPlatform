...
basePath = "dataset"
st = time.time()
filePath = os.path.join(basePath, "banner_trends_carr_daily.csv")
...
df_merge_clean["YEARS"] =df_merge_clean["REPORT_DATE"].apply(lambda report_date:datetime.datetime.strptime(report_date,"%Y-%m-%d").timetuple().tm_year) 
df_merge_clean["DAYS"] = df_merge_clean["REPORT_DATE"].apply(lambda report_date:datetime.datetime.strptime(report_date,"%Y-%m-%d").timetuple().tm_yday)
df_merge_clean = df_merge_clean.drop("REPORT_DATE", axis=1)
df_merge_clean.head()
