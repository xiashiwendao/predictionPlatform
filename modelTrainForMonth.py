...
# df_banner.head()
# df_banner.columns
# df_banner["BANNER_NAME"].value_counts()
...
df_banner_agg = df_banner_filted.groupby(['YEAR_OF_WEEK','MONTH_OF_YEAR']).mean().reset_index()#.sort_values(by=[""])
df_banner_agg.head(100)
# len(df_banner_agg)
from sklearn.metrics import mean_squared_error
from sklearn.model_selection import train_test_split
...
df_trend_carre["YEAR_OF_WEEK"] = df_trend_carre["REPORT_DATE"].apply(lambda x:x[0:4])
df_trend_carre["MONTH_OF_YEAR"] = df_trend_carre["REPORT_DATE"].apply(lambda x:x[5:7])
df_trend_carre["MONTH_OF_YEAR"]
# df_trend_carre.head(20)
# df_trend_carre.columns
df_trend_carre_filterd = df_trend_carre.drop(['REPORT_DATE','BANNER_NAME','PRODUCT_BKEY'], axis=1, inplace=False)
df_trend_carre_filterd.head()
df_trend_carre_agg = df_trend_carre_filterd.groupby(['YEAR_OF_WEEK','MONTH_OF_YEAR']).sum().reset_index()
# len(df_trend_carre_agg)
df_trend_carre_agg.head()
df_merge = df_banner_agg.merge(df_trend_carre_agg, on=['YEAR_OF_WEEK','MONTH_OF_YEAR'], how="inner")
# len(df_merge)
